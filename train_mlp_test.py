...
def preload_subset(batch_size, subset_percentage):
    ...
    xs = torch.stack([test_subset[i][0] for i in range(len(test_subset))])
    ys = torch.tensor([test_subset[i][1] for i in range(len(test_subset))])
    preloaded_dataset = torch.utils.data.TensorDataset(xs, ys)
...
def run_chunk(jobs, device, shared_tensor, preloaded, test_preloaded, seeds, model_class, optimizer, epochs):
...
def run_experiment(log2lr, width, seeds, job_id, device, shared_tensor, preloaded, test_preloaded, model_class, optimizer, epochs):
    train_dl = preloaded
...
if __name__ == '__main__':
    ...
    preloaded, test_preloaded = preload_subset(batch_size, args.subset)
    ...
    for enum, job_chunk in enumerate(jobs_chunks):
        ...
        p = mp.Process(target=run_chunk, args=(job_chunk, device, shared_tensor, preloaded, test_preloaded, seeds, model_class, optimizer, epochs))
        processes.append(p)
        p.start()
